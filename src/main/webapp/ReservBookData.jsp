	...
	<div class="login-container">
		<div class="login-box">
			...
			<span style="display: flex; flex-direction: row">
				<h2>Reserve Book</h2> <a href="issueBooks.jsp"><button
			...
			<form action="reservebook" method="post">
				...
				<c:forEach items="${rs.rows}" var="row">
					<div class="input-group">
						<label for="username">Name Of Student</label> <input type="text"
							id="username" name="studentName"
							value='<c:out value="${getName}"></c:out>' readonly>
					</div>
					<div class="input-group">
						<label for="username">Book</label> <input type="text"
							id="username" name="book"
							value='<c:out value="${row.book}"></c:out>' readonly>
					</div>
					<div class="input-group">
						<label for="username">Name Of Book</label> <input type="text"
							id="username" name="bookname"
							value='<c:out value="${row.BookName}"></c:out>' readonly>
					</div>
					<div class="input-group">
						<label for="username">Author Of Book</label> <input type="text"
							id="username" name="author"
							value='<c:out value="${row.Author}"></c:out>' readonly>
					</div>

					<div class="input-group">
						<label for="password">Edition</label> <input type="text"
							id="edition" name="edition"
							value='<c:out value="${row.edition}"></c:out>' readonly>
					</div>
					<div class="input-group">
						<label for="password">Enter Book Quantity You want</label> <input
							type="text" id="bookquantity" name="bookquantity" value="1"
							readonly>
					</div>
				</c:forEach>
				<sql:query dataSource="${ds}" var="ro">select returndate from issuebooks where status='Issued' and bookid='<c:out
						value="${a}"></c:out>';</sql:query>
				<div class="input-group">
					<label for="password">Select Issuedate According to You</label> <select
